...
def parse_args(args) -> Any:
    parser = argparse.ArgumentParser(
        description='Some filtering tools')
    parser.add_argument(
...
def filter(xyz_path: str, output_path: str):
    print("reading xyz file...")
    with open(xyz_path, "r") as f:
        lines = [line.strip().split(" ") for line in f.readlines()]

    filepath = os.path.join(output_path, "map_quality_roughness_filtered.xyz")
    f = open(filepath, "w")
    f.write("x y z roughness_filtered \n")
    is_data = False
    for line in lines:
        if line[0] == "DATA":
            is_data = True
        ...
        r = float(line[6])
        if r > 0 and r < 0.25:
            line_filtered = line[:3]
            line_filtered.append(line[6])
            f.write(' '.join(line_filtered))
            f.write("\n")
    f.close()

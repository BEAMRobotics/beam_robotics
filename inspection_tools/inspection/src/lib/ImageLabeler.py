...
def main(data_path, config_path, crack, delam, corrosion, spall, visualize, verbose):
    ...
    with open(config_path) as config_json:
        ...
        if crack:
            ...
            if not os.path.isfile(crack_model_path):
                print(
                    "ERROR: Crack model file does not exist. Crack segmentation will not be completed")
                crack = False
            # default = 2, crack and background classes
            num_classes = config_dict["crack_num_classes"]
            # default = 0.7
        ...
        if delam:
            ...
            if not os.path.isfile(delam_model_path):
                print(
                    "ERROR: Delam model file does not exist. Delam segmentation will not be completed")
                delam = False
        ...
        if corrosion:
            ...
            if not os.path.isfile(corrosion_model_path):
                print(
                    "ERROR: Corrosion model file does not exist. Corrosion segmentation will not be completed")
                corrosion = False
        ...
        if spall:
            ...
            if not os.path.isfile(spall_model_path):
                print(
                    "ERROR: Spall model file does not exist. Spall segmentation will not be completed")
                spall = False
    ...
    if crack:
        ...
        crack = False
        try:
            crack_checkpoint = torch.load(
                crack_model_path, map_location=device)
            crack_model.load_state_dict(crack_checkpoint["model"])
            crack_model.eval()
            crack = False
        except:
            print(
                "ERROR: Cannot load crack model file. Crack segmentation will not be completed")
    ...
    if delam:
        ...
        try:
            ...
            print(
                "ERROR: Cannot load delam model file. Delam segmentation will not be completed")
            delam = False
    ...
    with open(data_path + "CameraList.json") as camera_json:
    ...
    for camera in camera_list:
        ...
        for img_name in imgs_list:
            ...
            if img_info["is_bgr_image_set"]:
                mask_methods = []
                if verbose:
                    print(data_path + camera + "/" +
                          img_name + "/BGRImage.jpg")
                img = Image.open(data_path + camera + "/" +
                                 img_name + "/BGRImage.jpg").convert("RGB")
                    ...
                    with torch.no_grad():
                        ...
                        for i in range(len(output["scores"])):
                            if output["scores"][i] > crack_confidence_threshold:
                                mask = output["masks"][i]
                                mask = np.where(
                                    mask > crack_seg_threshold, 1, 0).astype(np.uint8)
                                crack_masks.append(mask)

                                if "mask_rcnn_crack" not in mask_methods:
                                    mask_methods.append("mask_rcnn_crack")

                    crack_mask = (np.logical_or.reduce(
                        crack_masks)).astype(np.uint8)
                ...
                mask_img = Image.fromarray(np.transpose(
                    np.stack([bgr_base, bgr_base, bgr_base], axis=1), [0, 2, 1]).astype(np.uint8))
                mask_img = mask_img.resize((width, height))
                save_path = os.path.join(
                    data_path + camera, img_name, "BGRMask.png")
                mask_img.save(save_path)

                if visualize:
                    bgr_base *= 50  # largest label is 4 will correspond to 200
                    mask_img_visualize = Image.fromarray(np.transpose(
                        np.stack([bgr_base, bgr_base, bgr_base], axis=1), [0, 2, 1]).astype(np.uint8))
                    mask_img_visualize = mask_img_visualize.resize(
                        (width, height))
                    mask_img_visualize.save(
                        data_path + camera + "/" + img_name + "/BGRMask_visualize.jpg")
            ...
            if img_info["is_ir_image_set"]:
                ...
                if delam:
                    img = np.asarray(Image.open(
                        data_path + camera + "/" + img_name + "/IRImage.jpg"))

                    img_scale = customRescale(img)

                    res = delam_model.predict(
                        np.expand_dims(img_scale, 0), batch_size=1)
                    labels = np.argmax(res.squeeze(), -1)
                    ir_mask = Image.fromarray((labels).astype(np.uint8))
                    ir_mask.save(data_path + camera + "/" +
                                 img_name + "/IRMask.png")

                    if visualize:
                        mask_visualize = Image.fromarray(
                            (labels*255).astype(np.uint8))
                        mask_visualize.save(
                            data_path + camera + "/" + img_name + "/IRMask_visualize.png")
...
if __name__ == "__main__":
    ...
    main(args.data_path, args.config, args.crack, args.delam,
         args.corrosion, args.spall, args.vis, args.verbose)

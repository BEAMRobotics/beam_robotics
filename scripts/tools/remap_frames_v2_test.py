...
bagInName = raw_input("Enter the path to the bag you want to edit the headers for")
...
target_topics = raw_input("Enter the topics you wish to rename a frame for (sepearted by a comma):").split(',')
target_frame_out = []
for position in range(len(target_topics)):
    target_frame_out.append(raw_input("What would you like to rename the frame of " + target_topics[position] + " to?"))
with bagOut as outbag:
    ...
    for topic, msg, t in bagIn.read_messages():
        if topic in target_topics:
            ...
            #print("Successfully renamed " + msg.header.frame_id + " to " + new_msg.header.frame_id)

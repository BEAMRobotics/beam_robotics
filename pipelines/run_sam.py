...
class ImageExtractorIterator:
    def __init__(self, image_extractor_path: str, camera_list_file="CameraList.json", cameras=[], image_filename="BGRImage.jpg"):
        ...
        self.camera_list_path = os.path.join(
            image_extractor_path, camera_list_file)
...
def use_image():
    while (True):
        user_input = input("label this image? (y/n or e to exit):").lower()
        if user_input == 'y':
            return True, False
        elif user_input == 'n':
            return False, False
...
def get_mask(predictor, image_path, image, image_bgr):
    ...
    with open(json_path, 'w') as f:
        json.dump(f, image_info, indent=4)
...
def main():
    ...
    while True:
        ...
        use_this_image, quit_script = use_image()
        if quit_script:
            print("quitting...")
            break

        if not use_this_image:
            continue

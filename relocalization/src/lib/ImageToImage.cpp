...
std::vector<std::tuple<Eigen::Vector2i, Eigen::Vector3d>>
    ImageToImage::Query(cv::Mat query_image) {
  ...
  std::vector<beam_cv::FeatureTrack> image_track =
      tracker_->OfflineTracker(images)[images.size() - 1];
  std::vector<std::tuple<Eigen::Vector2d, Eigen::Vector3d>> matches;

...
LidarCamCalib::LidarCamCalib(){
  cloud_transformed = boost::make_shared<PointCloud>();
  cloud_colored = boost::make_shared<PointCloudColor>();
  cloud = boost::make_shared<PointCloud>();
  viewer = boost::make_shared<Viz>("3D Viewer");
}

std::string LidarCamCalib::GetFilePath(std::string filename) {
  std::string file_location = __FILE__;
  file_location.erase(file_location.end() - 21, file_location.end());
  return file_location + "data/" + filename;
}

void LidarCamCalib::PrintIntructions() {
  std::cout
...
void LidarCamCalib::OutputUpdatedTransform() {
...
void LidarCamCalib::keyboardEventOccurred(
...
void  LidarCamCalib::run() {
  ...
  cam_intrinsics->LoadJSON(filename);
  ...
  projector.SetIntrinsics(cam_intrinsics.get());
  ...
  viewer->registerKeyboardCallback(this->keyboardEventOccurred, (void *)&viewer);

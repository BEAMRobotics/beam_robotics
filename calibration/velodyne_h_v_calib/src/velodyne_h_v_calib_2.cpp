...
void saveTFData() {
    ...
    os << ros::package::getPath("velodyne_h_v_calib") << "/results/" << year
       << "_" << month << "_" << day << ".yaml";
...
void loadParams() {
	...
	ros::param::get("T_HVLP_VVLP", transform_matrix_string);
...
{
	ros::init(argc, argv, "velodyne_h_v_calib_2");
	loadParams();
	ros::NodeHandle nh;
	ros::Time::init();
	...
	while (true) {
		...
		{
			ROS_WARN("VELODYNE HV TRANSFORM: Could not broadcast transform from %s to %s.", vvlpFrameId.c_str(), hvlpFrameId.c_str());
		}
		subscribeRate.sleep();
	}
}

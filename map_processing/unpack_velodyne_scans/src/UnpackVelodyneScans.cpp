...
UnpackVelodyneScans::UnpackVelodyneScans(const std::string& bag_file_path,
	...
	: bag_file_path_(bag_file_path), calibration_file_(calibration_file),
	...
	if (set_up!=0) { BEAM_CRITICAL("Unable to load calibration file."); }
	...
	container_ptr_ = boost::shared_ptr<velodyne_pointcloud::PointcloudXYZIR>(
		...
		target_frame_, fixed_frame_, data_->scansPerPacket()));

  container_ptr_->configure(max_range_, min_range_, fixed_frame_, target_frame_);
}
...
void UnpackVelodyneScans::Unpack() {
  ...
  for (auto iter = view.begin(); iter != view.end(); iter++) {
    ...
    auto vel_scan_msg = iter->instantiate<velodyne_msgs::VelodyneScan>();
		...
		beam::OutputPercentComplete(message_counter,total_messages,output_message);
  }
